...
import (
	...
	"github.com/rancher/rancher/pkg/webhook/mocks"
...
func (a *AggregateResolverSuite) TestAggregateRuleResolverGetRules() {
	...
	tests := []struct {
	...
	}{
		{
			...
			resolvers: func(t *testing.T) ([]validation.AuthorizationRuleResolver, Rules) {
				...
				resolver.EXPECT().VisitRulesFor(gomock.Any(), testUser, testNameSpace, gomock.Any()).
					DoAndReturn(func(_ context.Context, _ user.Info, _ string, visitor func(source fmt.Stringer, rule *rbacv1.PolicyRule, err error) bool) bool {
						for _, rule := range expectedRules {
							visitor(nil, &rule, nil)
						}
						return true
					})
				resolver.EXPECT().GetRoleReferenceRules(gomock.Any(), gomock.Any(), gomock.Any()).Return(expectedRules, nil)
		...
		},
		{
			...
			resolvers: func(t *testing.T) ([]validation.AuthorizationRuleResolver, Rules) {
				...
				resolver := mocks.NewMockAuthorizationRuleResolver(gomock.NewController(t))
				resolver.EXPECT().VisitRulesFor(gomock.Any(), gomock.Any(), testNameSpace, gomock.Any()).
					DoAndReturn(func(_ context.Context, _ user.Info, _ string, visitor func(source fmt.Stringer, rule *rbacv1.PolicyRule, err error) bool) bool {
						visitor(nil, nil, errNotFound)
						return true
					})
				resolver2 := mocks.NewMockAuthorizationRuleResolver(gomock.NewController(t))
				resolver2.EXPECT().VisitRulesFor(gomock.Any(), gomock.Any(), testNameSpace, gomock.Any()).
					DoAndReturn(func(_ context.Context, _ user.Info, _ string, visitor func(source fmt.Stringer, rule *rbacv1.PolicyRule, err error) bool) bool {
						visitor(nil, nil, errNotFound)
						return true
					})
		...
		},
		{
			...
			resolvers: func(t *testing.T) ([]validation.AuthorizationRuleResolver, Rules) {
				...
				resolver := mocks.NewMockAuthorizationRuleResolver(gomock.NewController(t))
				resolver.EXPECT().VisitRulesFor(gomock.Any(), testUser, testNameSpace, gomock.Any()).
					DoAndReturn(func(_ context.Context, _ user.Info, _ string, _ func(source fmt.Stringer, rule *rbacv1.PolicyRule, err error) bool) bool {
						return true
					})
				resolver.EXPECT().GetRoleReferenceRules(gomock.Any(), gomock.Any(), gomock.Any()).Return(expectedRules, nil)
				resolver2 := mocks.NewMockAuthorizationRuleResolver(gomock.NewController(t))
				resolver2.EXPECT().VisitRulesFor(gomock.Any(), testUser, testNameSpace, gomock.Any()).
					DoAndReturn(func(_ context.Context, _ user.Info, _ string, visitor func(source fmt.Stringer, rule *rbacv1.PolicyRule, err error) bool) bool {
						for _, rule := range expectedRules {
							visitor(nil, &rule, nil)
						}
				...
				resolver2.EXPECT().GetRoleReferenceRules(gomock.Any(), gomock.Any(), gomock.Any()).Return(nil, nil)
				return []validation.AuthorizationRuleResolver{resolver, resolver2}, expectedRules
			},
		},
		{
			...
			resolvers: func(t *testing.T) ([]validation.AuthorizationRuleResolver, Rules) {
				...
				resolver.EXPECT().GetRoleReferenceRules(gomock.Any(), gomock.Any(), gomock.Any()).Return(expectedRules1, nil)
				resolver2 := mocks.NewMockAuthorizationRuleResolver(gomock.NewController(t))
				resolver2.EXPECT().VisitRulesFor(gomock.Any(), testUser, testNameSpace, gomock.Any()).
					DoAndReturn(func(_ context.Context, _ user.Info, _ string, visitor func(source fmt.Stringer, rule *rbacv1.PolicyRule, err error) bool) bool {
				...
				resolver2.EXPECT().GetRoleReferenceRules(gomock.Any(), gomock.Any(), gomock.Any()).Return(expectedRules2, nil)
	...
	for _, tt := range tests {
		a.Run(tt.name, func() {
			...
			gotRules, err := agg.RulesFor(context.Background(), tt.user, tt.namespace)
			if tt.wantErr {
			...
			gotRules, err = agg.GetRoleReferenceRules(context.Background(), rbacv1.RoleRef{}, tt.namespace)
			if !expectedRules.Equal(gotRules) {
				a.Fail("List of rules did not match", "wanted=%+v got=%+v", expectedRules, gotRules)
			}
